...
ESCALAS = {
    "refrigerante": {
        "nome":  "Refrigerante",
        "icone": "🥤",
        "ideal_max": 10,   # abaixo disso = ideal, sem problema
        "check": lambda t: (
            "BOM, mas acima do ideal" if t <= 18 else
            "ATENÇÃO"                 if t <= 25 else
            "CRÍTICO"
        ) if t > 10 else None
    },
    "acai": {
        ...
        "ideal_max": 5,
        "check": lambda t: (
            "BOM, mas acima do ideal" if t <= 14 else
            "ATENÇÃO"                 if t <= 20 else
            "CRÍTICO"
        ) if t > 5 else None
    },
    "pizza": {
        "nome":  "Pizza",
        "icone": "🍕",
        "ideal_min": 30,   # pizza é inversa: quanto mais quente melhor
        "check": lambda t: (
            "BOM"     if t >= 28 else
            "ATENÇÃO" if t >= 25 else
            "CRÍTICO"
